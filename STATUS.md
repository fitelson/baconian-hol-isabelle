...
The precise Isabelle target, once the full-domain predicate is defined, is a
finite decision basis:

```isabelle
theorem pp_seed_Henkin_domain_attains_decided:
  assumes "pp_seed_Henkin_prop_domain r D"
  shows "EX S. S <= D & finite S &
    (ALL i. (ALL X : S. i : pp_decided X) --> (ALL X : D. i : pp_decided X))"
```

The tempting stronger form `ALL X : D. pp_decided r <= pp_decided X`, which
would make `{r}` attain as it does in the Boolean-modal fragment, is exactly
where the induction breaks: bound variables range over propositions not
necessarily decided wherever `r` is.
