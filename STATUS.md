...
### One nowhere-decided proposition kills it
...
is already empty at that single element and attainment holds. And such
propositions are not exotic --- every parity proposition is one, since its only
views are itself and its complement and neither is `{}` or `UNIV`:
...
The counterexample domain escapes only because each `C_n` *is* decided
somewhere, namely at every world deeper than `n` (`pp_shortprop_not_nowhere_decided`).

### The decisive point: finitely generated domains always attain

Decidedness passes through the Boolean connectives and the modality, so a world
deciding a generating family decides everything the family generates:
...
its generators. In particular the Boolean-and-modal closure of a single seed
always attains, and **the attaining element is the seed itself**.

### Verdict

The counterexample cannot be realized from the Boolean and modal closure of a
seed. Any realization must use the rest of the language --- identity, the
quantifiers, `Pure` --- to manufacture infinitely many propositions no finite
subfamily of which decides the rest, while simultaneously avoiding every
nowhere-decided proposition. Note the asymmetry: refuting realization needs a
single witness, whereas establishing it needs a universal claim about the whole
closure. The burden is on realization, and the counterexample of
`Bacon_PP_Attainment_Failure.thy` should continue to be read as a statement
