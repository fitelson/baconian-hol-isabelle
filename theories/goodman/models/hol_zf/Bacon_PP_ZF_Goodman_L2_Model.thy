...
definition pp_b_exact_strong_L2_pair ::
    "pp_b_operator \<Rightarrow> pp_b_operator \<Rightarrow> bool" where
  "pp_b_exact_strong_L2_pair X Y \<longleftrightarrow>
    X \<in> pp_b_exact_stock \<and>
    Y \<in> pp_b_exact_stock \<and>
    (\<forall>p q.
      pp_b_exact_fun_prime p \<longrightarrow>
      pp_b_exact_fun_prime q \<longrightarrow>
      X p = Y q \<longrightarrow>
      (\<exists>Z \<in> pp_b_exact_G.
        X = Y \<circ> Z \<and> q = Z p))"

definition pp_b_exact_strong_L2 :: bool where
  "pp_b_exact_strong_L2 \<longleftrightarrow>
    (\<forall>X \<in> pp_b_exact_stock.
      \<forall>Y \<in> pp_b_exact_stock.
        pp_b_exact_strong_L2_pair X Y)"

lemma pp_b_exact_strong_L2_imp_L2:
  assumes "pp_b_exact_strong_L2"
  shows "pp_b_exact_L2"
  using assms
  unfolding pp_b_exact_strong_L2_def
    pp_b_exact_strong_L2_pair_def
    pp_b_exact_L2_def pp_b_exact_L2_pair_def
    pp_b_exact_same_kind_def
  by blast
