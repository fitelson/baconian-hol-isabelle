theory Bacon_PP_ZF_Tree_Modal_Boolean_Model
  imports
    Higher_Order_Metaphysics_PP_ZF_Modal_Boolean_Higher_Types.Bacon_PP_ZF_Tree_Modal_Boolean_Higher_Types
begin

section \<open>Seeded constants for the modal-Boolean stock\<close>

definition pp_t_probe_modal_boolean_model_constants ::
    "string \<Rightarrow> otype \<Rightarrow> ZF"
where
  "pp_t_probe_modal_boolean_model_constants =
    pp_t_seeded_internal_constants
      pp_t_probe_modal_boolean_model_pure
      pp_t_probe_modal_boolean_stock_seed_at"

lemma pp_t_probe_modal_boolean_seeded_fundamental_admissible:
  "pp_t_predicate_admissible \<sigma>
    (pp_t_seeded_fundamental_at
      pp_t_probe_modal_boolean_stock_seed_at \<sigma>)"
proof (cases \<sigma>)
  case Ind
  then show ?thesis
...
next
  ...
  proof (intro allI impI)
    ...
    have seed:
        "Elem (pp_t_probe_modal_boolean_stock_seed_at v)
          (pp_t_domain Prop)"
      by (rule pp_t_probe_modal_boolean_stock_seed_at_in_domain)
    have refl:
        "pp_t_eqv Prop v
          (pp_t_probe_modal_boolean_stock_seed_at v)
          (pp_t_probe_modal_boolean_stock_seed_at v)"
      by (rule pp_t_eqv_reflexive[OF seed])
    show "pp_t_seeded_fundamental_at
          pp_t_probe_modal_boolean_stock_seed_at Prop v x
        =
        pp_t_seeded_fundamental_at
          pp_t_probe_modal_boolean_stock_seed_at Prop v y"
...
interpretation ProbeModalBooleanModelConstants:
  pp_t_constants pp_t_probe_modal_boolean_model_constants
proof
  fix c \<sigma>
  show "Elem (pp_t_probe_modal_boolean_model_constants c \<sigma>)
      (pp_t_domain \<sigma>)"
  proof (cases \<sigma>)
    case Ind
    then show ?thesis
      unfolding pp_t_probe_modal_boolean_model_constants_def
  ...
  next
    ...
    have pure_classifier:
      ...
      by (rule pp_t_classifier_in_domain)
        (rule pp_t_probe_modal_boolean_model_pure_admissible)
    have fun_classifier:
        "Elem
          (pp_t_classifier \<sigma>
            (pp_t_seeded_fundamental_at
      ...
      by (rule pp_t_classifier_in_domain)
        (rule
          pp_t_probe_modal_boolean_seeded_fundamental_admissible)
    have default:
    ...
    show ?thesis
      ...
      by (auto simp: pp_t_probe_modal_boolean_model_constants_def
          pp_t_seeded_fundamental_at.simps)
  qed
qed

lemma pp_t_probe_modal_boolean_model_eval_Pure[simp]:
  "pp_t_eval pp_t_probe_modal_boolean_model_constants \<rho>
      (pp_Pure \<sigma>) =
    pp_t_classifier \<sigma>
      (pp_t_probe_modal_boolean_model_pure \<sigma>)"
  by (simp add: pp_t_probe_modal_boolean_model_constants_def
      pp_Pure_def pp_pure_name_def)

lemma pp_t_probe_modal_boolean_model_eval_Fun[simp]:
  "pp_t_eval pp_t_probe_modal_boolean_model_constants \<rho>
      (pp_Fun \<sigma>) =
    pp_t_classifier \<sigma>
      (pp_t_seeded_fundamental_at
...
lemma pp_t_probe_modal_boolean_model_eval_pure_holds:
...
proof -
  have argument:
      "Elem
        (pp_t_eval pp_t_probe_modal_boolean_model_constants \<rho> M)
        (pp_t_domain \<sigma>)"
    using ProbeModalBooleanModelConstants.pp_t_eval_type[OF typed env]
    by (simp add: pp_t_dom_def)
  show ?thesis
    unfolding pp_pure_def
    using pp_t_classifier_holds[
      OF argument,
      of "pp_t_probe_modal_boolean_model_pure \<sigma>" w]
    by simp
qed

lemma pp_t_probe_modal_boolean_model_eval_fun_holds:
  assumes typed: "\<Gamma> \<turnstile> M : \<sigma>"
    and env: "pp_t_env_typed \<Gamma> \<rho>"
  shows "pp_t_holds
      (pp_t_eval pp_t_probe_modal_boolean_model_constants \<rho>
    ...
    \<longleftrightarrow>
    pp_t_seeded_fundamental_at
      pp_t_probe_modal_boolean_stock_seed_at \<sigma> w
      (pp_t_eval pp_t_probe_modal_boolean_model_constants \<rho> M)"
proof -
  have argument:
      "Elem
...
lemma pp_t_probe_modal_boolean_model_closed_logical_eval_eqv:
