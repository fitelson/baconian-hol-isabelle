...
constant falsity, and the constant-function builder \(K\).  Consequently
every subcollection of Goodman's principles whose logical-purity instances
are confined to those five formulas is consistent in Bacon's proof system.
...
five later \(K\)-fragment and bridge results.  A fresh build passes.  None of
these proofs contains an admitted lemma, an untrusted proof step, or an
...
next planned extension is purity of conjunction; a complete result requires
all closed logical terms at all types.
...
logical term at every type.  The next model extension is conjunction, followed
by a uniform treatment of curried truth-functions.
...
constant falsity, and \(K\).  The next extension should add purity of
conjunction, followed by a uniform treatment of curried truth-functions.
...
\item A model validating \PP\ when the logical-purity schema extends beyond
      identity, negation, constant truth, constant falsity, and \(K\).
      The next case is conjunction, followed by uniform curried
      truth-functions; Goodman's full question requires all closed logical
      terms at all types.
...
\item \textbf{Continue adding logical purity.}  Identity, negation, constant
      truth, constant falsity, and \(K\) are now handled.  Add purity of
      conjunction, then prove a uniform result for curried truth-functions,
      while preserving closure of purity under application, the verified
      zeroary and unary instances of Recombination and Exhaustion, Modalized
      Functionality, and \PP.  A complete result requires every closed
      logical term at every type.
...
\begin{verbatim}
/opt/homebrew/bin/isabelle build -v -d . \
...
truth, constant falsity, and \(K\), so every subcollection whose
logical-purity instances are confined to those five formulas is consistent.
